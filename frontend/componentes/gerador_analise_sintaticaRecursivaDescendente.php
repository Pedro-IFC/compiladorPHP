    ...
    <div class="table-lex-result">
        <?php
            if(isset($_GET['string'])){
                $AnalisadorDR = New AnalisadorSintaticoDescenteRecursive(new Lexical($_GET['string']));
                if($AnalisadorDR->analisar($_GET['inicio'])){
                    ?>
                        <h3 style="color: green">Passou da análise sintatica!</h3>
                        <h3>Árvore de derivação: </h3>
                    <?php
                        $AnalisadorDR->imprimirArvore();
                }else{
                    ?>
                        <h3 style="color: red">Não passou na análise sintática!</h3>
                    <?php
                    if(!empty($AnalisadorDR->getErros())){
                        ?>
                        <table class="tokens">
                            <?php 
                            foreach($AnalisadorDR->getErros() as $erro){
                                ?>
                                <tr class="token">
                                    <td class="name reconhecido"><?= $erro ?></td>
                                </tr>
                                <?php
                            }
                            ?>
